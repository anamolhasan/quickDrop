...
export default function RiderApplicationsPage() {
  ...
  const handleAccept = async (id) => {
    try {
      const res = await fetch(`${apiUrl}/admin/riders/${id}`, { // ✅ FIXED: Correct endpoint
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          // ✅ REMOVED: Authorization header
          "user-id": session.user.id, // ✅ ADDED: User ID
        },
        body: JSON.stringify({ status: "accepted" }),
      });
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({ message: "Failed to accept rider" }));
  ...
  const handleRemove = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({ message: "Failed to remove rider" }));
        toast.error(errorData.message);
      }
    } catch (error) {
      console.error("Error removing rider:", error);
